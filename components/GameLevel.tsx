...
import { Question, LevelStatus, Difficulty, BossInfo, QuestionSource, STAGES_PER_CHAPTER } from '../types';
import { generateBossAvatar } from '../services/geminiService';
import { getQuestion } from '../services/questionService';
import { getChapterData } from '../chapterData';
import { LoadingSpinner } from './LoadingSpinner';
import { playSound, playBGM, stopBGM } from './SoundManager';
import { Check, X, Sword, Scroll, ArrowRight, Heart, Sparkles, AlertCircle, Shield, Flag, Save, Map as MapIcon } from 'lucide-react';
...
const GameLevel: React.FC<GameLevelProps> = ({ levelStatus, questionSource, onComplete, onBack }) => {
  ...
  // Track questions already used in this session to prevent duplicates
  const [usedQuestionTexts, setUsedQuestionTexts] = useState<string[]>([]);

  const [showDefeatModal, setShowDefeatModal] = useState(false);

  const isBoss = levelStatus.stage === STAGES_PER_CHAPTER;
  ...
  // Unified load function to ensure history is updated correctly
  const loadNextQuestion = useCallback(async (currentHistory: string[], currentHP: number) => {
    setLoading(true);
    setQuestion(null);
    ...
    try {
      ...
      if (isBoss) {
          ...
          if (currentHP === -1) { 
              ...
              // First load will trigger again via HP change, so we stop here
              return; 
          }
          ...
          if (cachedImage) {
              ...
              const img = await generateBossAvatar(currentBoss.visualPrompt);
              if (img) {
                  setBossImage(img);
      ...
      // CRITICAL: Pass currentHistory explicitly to avoid stale state issues
  ...
  }, [levelStatus, isBoss, questionSource, bossImage, imageLoading]);

  // Initial load or HP change load
  useEffect(() => {
    if (!showDefeatModal) {
        loadNextQuestion(usedQuestionTexts, bossHP);
    }
  }, [levelStatus.chapter, levelStatus.stage, bossHP, questionSource, loadNextQuestion]);
  ...
  const handleNext = () => {
    if (question && selectedOption === question.correctAnswerIndex) {
      // 1. Prepare updated history immediately
      const nextHistory = [...usedQuestionTexts, question.text];
      setUsedQuestionTexts(nextHistory);
      ...
      if (isBoss) {
          ...
          if (newHP <= 0) {
              ...
              setBossHP(newHP);
              // Loading will be triggered by useEffect watching bossHP
          }
      } else {
          onComplete(true);
      }
    } else {
      // If wrong, or something else, stay or fail
      onComplete(false);
    }
  };
  ...
  return (
    <div className={`max-w-2xl mx-auto w-full p-4 animate-fadeIn relative ${shake ? 'animate-shake' : ''}`}>
      <style>{`
        @keyframes shake {
      ...
      {showDefeatModal && bossInfo && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4 animate-fadeIn">
            <div className="relative bg-[#3E2723] border-4 border-[#FFB300] rounded-2xl max-w-lg w-full p-8 text-center shadow-[0_0_50px_rgba(255,179,0,0.5)] overflow-hidden">
                ...
                <h2 className="text-5xl md:text-6xl font-calligraphy text-[#FFB300] mb-4 animate-bounce drop-shadow-md">讨伐成功</h2>
                <div className="text-[#FFECB3] text-sm mb-8 font-serif tracking-widest flex items-center justify-center gap-2">
                    <Save className="w-4 h-4" /> 进度已保存
                </div>
                ...
                <div className="bg-[#5D4037] p-4 rounded-xl border border-[#8D6E63] mb-8 shadow-inner">
                     <p className="text-[#FFE0B2] font-serif text-lg italic leading-relaxed">“{bossInfo.defeatQuote}”</p>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    <button 
                        ...
                        className="btn-game w-full py-4 text-xl font-bold bg-[#FFB300] text-[#3E2723] border-[#FFA000] hover:brightness-110 active:scale-95 transition-transform flex items-center justify-center gap-3"
                    >
                        <MapIcon className="w-6 h-6" /> 凯旋班师 (回选关页)
                    </button>
                    <p className="text-[#A1887F] text-xs">点击上方按钮，返回地图开启下一回征程</p>
                </div>
            </div>
        </div>
      )}
      ...
      <div className="bg-[#795548] text-[#FFF8E1] rounded-xl p-3 shadow-md border-b-4 border-[#5D4037] mb-6 flex justify-between items-center relative overflow-hidden">
        ...
        <div className="z-10 flex items-center gap-2">
            <span className={`px-3 py-1 rounded text-[10px] font-bold border ${questionSource === 'local' ? 'bg-[#66BB6A] border-[#2E7D32]' : 'bg-[#FFB300] border-[#FF6F00]'} text-white`}>
                {questionSource === 'local' ? '本地' : 'AI'}
            </span>
            <span className={`px-3 py-1 rounded text-xs font-bold border shadow-sm ${isBoss ? 'bg-[#D32F2F] border-[#B71C1C] text-white' : 'bg-[#FFF8E1] border-[#FFB300] text-[#795548]'}`}>
                {isBoss ? 'BOSS WAR' : `STAGE ${levelStatus.stage}/${STAGES_PER_CHAPTER}`}
            </span>
        </div>
      </div>
           ...
           <div className="absolute -top-3 z-20 flex gap-2 bg-[#3E2723] px-4 py-1.5 rounded-full border-2 border-[#5D4037] shadow-lg">
                {Array.from({length: maxBossHP > 0 ? maxBossHP : 0}).map((_, i) => (
                    <div key={i} className="relative w-6 h-6 flex items-center justify-center">
                        <Heart className={`w-full h-full drop-shadow-md transition-all duration-500 ${i < bossHP ? 'text-[#F4511E] fill-[#F4511E]' : 'text-[#5D4037] fill-[#4E342E]'}`} />
                    </div>
                ))}
           </div>
           <div className={`relative w-48 h-48 rounded-full border-8 border-[#FFB300] bg-white overflow-hidden shadow-2xl ${flash ? 'boss-hit border-[#F4511E]' : ''}`}>
               {imageLoading && !bossImage ? (
                  <div className="w-full h-full flex items-center justify-center bg-[#EFEBE9]"><Sword className="w-12 h-12 text-[#BCAAA4] animate-pulse" /></div>
                ) : bossImage ? (
        ...
        <>
          <div className="scroll-bg p-6 md:p-8 mb-6 text-[#3E2723]">
            <div className="flex justify-between items-start mb-4">
                <h2 className="text-xl md:text-2xl font-bold font-serif leading-relaxed text-shadow flex-1">{question.text}</h2>
                {!isAnswered && (
                    <div className="flex gap-2 ml-2">
                        <button onClick={useReinforcement} disabled={reinforcementUsed} className={`shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all shadow-sm ${reinforcementUsed ? 'bg-[#EFEBE9] border-[#D7CCC8] text-[#BCAAA4]' : 'bg-[#FFEBEE] border-[#FFCDD2] text-[#D32F2F] hover:scale-110 active:scale-95'}`} title="请求援军"><Flag className="w-5 h-5" /></button>
                        <button onClick={useHint} disabled={hintUsed} className={`shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all shadow-sm ${hintUsed ? 'bg-[#EFEBE9] border-[#D7CCC8] text-[#BCAAA4]' : 'bg-[#E3F2FD] border-[#90CAF9] text-[#1976D2] hover:scale-110 active:scale-95'}`} title="锦囊妙计"><Sparkles className="w-5 h-5" /></button>
                    </div>
                )}
            </div>
            ...
            {showHint && (
                <div className="mb-6 p-3 bg-[#E3F2FD]/50 border border-[#90CAF9] rounded text-[#1565C0] text-sm flex gap-3 animate-fadeIn items-start">
                    ...
                    <div className="flex-1"><span className="font-bold block text-[#1976D2] text-xs mb-1">孔明观星</span>{question.hint}</div>
                </div>
            )}
            ...
            <div className="space-y-4">
              {question.options.map((option, idx) => {
                ...
                return (
                  <button key={idx} onClick={() => handleOptionClick(idx)} disabled={isAnswered || isEliminated} className={`w-full min-h-[60px] text-left p-4 rounded-lg border-b-4 transition-all duration-100 flex items-center gap-4 relative overflow-hidden ${stateClass}`}>
                    {icon}<span className={`font-medium text-lg font-serif ${isEliminated ? 'line-through decoration-2 decoration-[#BDBDBD]' : ''}`}>{option}</span>
                  </button>
                );
              })}
            </div>
          ...
          {isAnswered && (
            <div className={`p-5 rounded-xl mb-24 animate-slideUp shadow-xl border-l-4 ${isCorrect ? 'bg-[#E8F5E9] border-[#66BB6A]' : 'bg-[#FBE9E7] border-[#FF5722]'}`}>
              <div className="mb-4">
                ...
                <p className="text-[#4E342E] text-sm leading-relaxed italic border-t border-[#3E2723]/10 pt-2">{question.explanation}</p>
              </div>
              <button onClick={handleNext} className={`w-full py-4 rounded-lg font-bold flex items-center justify-center gap-2 shadow-lg transition-transform active:scale-95 text-lg ${isCorrect ? 'btn-game text-[#3E2723] bg-[#FFF8E1] border-[#FFB300]' : 'bg-[#FF5722] text-white border-b-4 border-[#BF360C] hover:bg-[#F4511E]'}`}>
                {isCorrect ? <>{isBoss && bossHP > 1 ? "继续交战" : "乘胜追击"} <ArrowRight className="w-5 h-5" /></> : <>重整旗鼓</>}
              </button>
            </div>
          )}
        </>
